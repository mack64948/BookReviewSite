...
const chapterSummaryDirectory = path.join(process.cwd(), 'data/chapter_summaries');
...
export function getSortedSynopsisData() {
  // Get file names under /posts
  const chapterNumbers = getChapterNumbers();
  const allChaptersData = chapterNumbers.map((number) => {
    
    // Read markdown file as string
    const fullPath = path.join(chapterSummaryDirectory, number.toString() + ".md");
...
export async function getChapterData(number) {
  const fullPath = path.join(chapterSummaryDirectory, `${number}.md`);
  const fileContents = fs.readFileSync(fullPath, 'utf8');
